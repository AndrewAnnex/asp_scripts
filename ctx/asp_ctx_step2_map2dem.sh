...
for i in $( cat stereodirs.lis ); do
    ...
    point2dem --threads 16 --t_srs "${proj}" -r mars --nodata -32767 -s 18 ${i}_map_ba-PC.tif --orthoimage -n --errorimage ${i}_map_ba-L.tif -o dem/${i}_map_ba
    if [ $? -ne 0 ]
    then
        echo "Failure running point2dem at 18m/p for $i at $(date)"
        exit 1
    else
        echo "Success running point2dem at 18m/p for $i at $(date)"
    fi
    # Generate hillshade (useful for getting feel for textural quality of the DEM)
